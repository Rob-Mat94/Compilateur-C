...
type arg =
  ...
  | ARI of intLit*reg (** adresse pointée par un registre décalé par une constante*)
  | ARL of string*reg (** adresse pointée par un registre décalé par un label *)
...
let replace_holl t a r i l =
  map_arg (function
           | HollArg -> a
           | x -> replace_holl_arg r i l x) t
       
let print_programme o l =
  List.iter (print_instruction o) (List.rev l)
