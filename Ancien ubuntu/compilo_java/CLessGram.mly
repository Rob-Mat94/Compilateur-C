...
let get_setArray (_,_,_) = (*SetArray (x,y,z)*) raise Parse_error
let get_reference _ = raise Parse_error (* Ref x *)
...
postfix_expression:
  ...
  | identifier INC_OP { BinaryOperator( Set($1,BinaryOperator(Var $1,Add,IntegerLiteral 1) ) ,Sub,IntegerLiteral 1 )  }
  | identifier DEC_OP { BinaryOperator( Set($1,BinaryOperator(Var $1,Sub,IntegerLiteral 1) ) ,Add,IntegerLiteral 1)  }
        ;
...
unary_expression:
        ...
        | INC_OP identifier { Set($2,BinaryOperator(Var $2, Add, IntegerLiteral 1)) }
        | DEC_OP identifier { Set($2,BinaryOperator(Var $2, Sub, IntegerLiteral 1)) }
        | unary_operator cast_expression
        {
          let  c = $1 in
          match c with
              ADD_CHR -> $2
            | SUB_CHR -> UnaryOperator (MinusM, $2)
            | BANG_CHR -> UnaryOperator (Not, $2)
            | STAR_CHR -> UnaryOperator (get_dereference (), $2)
            | _-> failwith "error"
            }
        ;
...
relational_expression:
        ...
        {
        BinaryOperator( $1, LL , $3)
        }
        | relational_expression CLOSE_ANGLE_CHR shift_expression
        {
                BinaryOperator( $3, LL , $1)
        }
        | relational_expression LE_OP shift_expression
        {
                BinaryOperator( $1, LE , $3)
        }
        | relational_expression GE_OP shift_expression
        {
                BinaryOperator( $3, LE , $1)
        }
        ;
...
equality_expression:
        ...
        {
                BinaryOperator( $1, EQ , $3)
        }
        | equality_expression NE_OP relational_expression
        {
                BinaryOperator( $1, NEQ , $3)
        }
        ;
         ...
         unary_expression EQ_CHR assignment_expression
            {
             ...
             match left with
               Var x -> Set (x, $3)
             | UnaryOperator (uop,e) when (try uop = get_dereference () with _-> false)
               -> BinaryOperator (e,get_setReference (),$3)
             | BinaryOperator (Var x, bop , i) when (try bop = get_index () with _-> false)
               -> get_setArray (x, i, $3)
             |_ -> failwith "error"
           }
        ;
...
glob_var_declaration:
        type_specifier identifier SEMI_CHR  { VarDecl($2,None) }
    |   type_specifier identifier EQ_CHR CONSTANT SEMI_CHR  { VarDecl($2,Some $4) }
