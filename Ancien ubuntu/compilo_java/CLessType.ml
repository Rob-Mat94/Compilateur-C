...
type bop =
  Mult (** multiplication i.e. [*] *) | Add (** addition i.e. [+] *)| Div (** division i.e. [/] *) | Sub (** soustraction i.e. [-] *)| Mod (** modulo i.e. [%] *)
  | And (** et booléen i.e. [&&] *) | Or (** ou booléen i.e. [||] *)
  | EQ  (** égale i.e. [==] *)
  | NEQ (** différent i.e. [!=] *)
...
| UnaryOperator of uop * expression (** application d'opérateur unaire *)
| BinaryOperator of expression * bop * expression (** application d'opérateur binaire *)
| Seq of expression list (** séquence d'expression i.e. [Seq([e1;e2;...])] -> e1;e2;... *)
| StringLiteral of string (** string littéral i.e. chaine de caractères constante *)
...
et liste de statement *)
  ...
  | IfStmt of expression * statement * statement (** bloc if-then-else: [IfStmt(e,s1,s2)] -> if(e)\{s1\}else\{s2\} *)
  | WhileStmt of expression * statement (** bloc while: [WhileStmt(e,s)] -> while(e)\{s\} *)
  | ReturnStmt of expression option (** retour d'une fonction *)

type declaration =
  | VarDecl of string * (int option)  (** définition de variable global *)
  | FunctionDecl of string * ( string list ) * statement (** définition de fonction *)
...
let rec iter_fun_expr f e =
  ...
  | None -> (
    match e with
    | UnaryOperator(mop,e) -> UnaryOperator(mop,iter_fun_expr f e)
    | BinaryOperator(e1,bop,e2) -> BinaryOperator(iter_fun_expr f e1,bop,
                                        iter_fun_expr f e2)
...
let rec iter_fun_stat f g s =
  ...
  | None -> (
    ...
    | IfStmt(e,s1,s2) -> IfStmt(iter_fun_expr f e,
                                iter_fun_stat f g s1,
                                iter_fun_stat f g s2)
    | WhileStmt(e,s) -> WhileStmt(iter_fun_expr f e,
                                  iter_fun_stat f g s)
