...
(*
 *	C (c) 2005 b L S©c e V©r (L),
 *	U 8643 C & E C.
 *	W b J G-L.  D f t c p.
 *
 *	P i g t a t u t s f a
 *	p o a c s, a t r i f,
 *	s t t f r:
 *
 *	1. N t a n i e i r f t c o u o
 *		t s, n m h a, e i t a
 *		f d i i.
 *
 *	2. T o o t s m n b m, e
 *		b e c o b o.
 *
 *	3. A v m b p m a s, a m n
 *		b m a b t o s.
 *
 *	4. T s i r t n-c u o.  C
 *		u i s t a s l, o f L.
 *
*)
...
let get_index ()= Index (* r P*)
let get_setArray (x,y,z) = SetArray (x,y,z) (*r P*)
let get_reference x = Ref x (*r P (* R x *)*)
let get_dereference () = (*r P*) Deref
let get_setReference () = (*r P *) SetReference
...
; (fun __caml_parser_env ->
    ...
    Obj.repr(
# 96 "CLessGram.mly"
        ( BinaryOperator (_1,get_index (), _3) )
# 689 "CLessGram.ml"
               : 'postfix_expression))
; (fun __caml_parser_env ->
...
; (fun __caml_parser_env ->
    ...
    Obj.repr(
# 105 "CLessGram.mly"
                      ( BinaryOperator( Set(_1,BinaryOperator(Var _1,Add,IntegerLiteral 1) ) ,Sub,IntegerLiteral 1 )  )
# 717 "CLessGram.ml"
               : 'postfix_expression))
; (fun __caml_parser_env ->
    let _1 = (Parsing.peek_val __caml_parser_env 1 : 'identifier) in
    Obj.repr(
# 106 "CLessGram.mly"
                      ( BinaryOperator( Set(_1,BinaryOperator(Var _1,Sub,IntegerLiteral 1) ) ,Add,IntegerLiteral 1)  )
# 724 "CLessGram.ml"
               : 'postfix_expression))
; (fun __caml_parser_env ->
...
; (fun __caml_parser_env ->
    ...
    Obj.repr(
# 119 "CLessGram.mly"
                            ( Set(_2,BinaryOperator(Var _2, Add, IntegerLiteral 1)) )
# 754 "CLessGram.ml"
               : 'unary_expression))
; (fun __caml_parser_env ->
    let _2 = (Parsing.peek_val __caml_parser_env 0 : 'identifier) in
    Obj.repr(
# 120 "CLessGram.mly"
                            ( Set(_2,BinaryOperator(Var _2, Sub, IntegerLiteral 1)) )
# 761 "CLessGram.ml"
               : 'unary_expression))
; (fun __caml_parser_env ->
...
# 122 "CLessGram.mly"
        (
          ...
          match c with
              ADD_CHR -> _2
            | SUB_CHR -> UnaryOperator (MinusM, _2)
            | BANG_CHR -> UnaryOperator (Not, _2)
            | STAR_CHR -> UnaryOperator (get_dereference (), _2)
            | _-> failwith "error"
            )
# 777 "CLessGram.ml"
               : 'unary_expression))
...
# 249 "CLessGram.mly"
            (
             ...
             | UnaryOperator (uop,e) when (try uop = get_dereference () with _-> false)
               -> BinaryOperator (e,get_setReference (),_3)
