...
let rec print_list_clang pre f o = function
    [] -> ()
  | t::[] -> f pre '`' o t
  | t::q -> f pre '-' o t; print_list_clang pre f o q
...
let rec  print_expression o = function
  ...
  | UnaryOperator(m,e) ->
     fprintf o "@[<hv 2>UnaryOperator@ (%a,%a)@]"
             print_mop m
             print_expression e
  | BinaryOperator(e,Index,i) ->
     fprintf o "%a[%a]" print_expression e
       print_expression i
  | BinaryOperator(e1,b,e2) ->
     fprintf o "@[<hv 2>BinaryOperator@ (%a,%a,%a)@]" print_expression e1
                    print_bop b
                    print_expression e2
  | Seq(el) -> fprintf o "[%a]" (print_list ";@;" print_expression) el
  | StringLiteral(s) -> fprintf o ("\"%s\"") (String.escaped s)
...
let rec print_dot_expression o id = function
      ...
      | Ref s -> Printf.fprintf o "Ref \"%s\"" s; id+1
      | IntegerLiteral i -> Printf.fprintf o  "id%i [label=\"%i\"]\n" (id+1) i; id+1
      | StringLiteral(s) -> Printf.fprintf o  "id%i [label=\"%s\"]\n" (id+1) (String.escaped s); id+1
...
let rec print_declaration o = function
  ...
  | FunctionDecl(s,dl,stat) -> fprintf o "@[<hv 2>FunctionDecl(\"%s\",[%a],%a)@]" s
...
and print_statement o = function
  ...
  | IfStmt(e,s1,s2) ->
     fprintf o "@[<hv 2>IfStmt(%a,@,%a,@,%a)@]"
             print_expression e
             (print_statement) s1
             (print_statement) s2
  | WhileStmt(e,s) -> fprintf o "@[<hv 2>WhileStmt(%a,@,%a)@]"
                              print_expression e
                              print_statement s
  | ReturnStmt(None) -> fprintf o "@[<hv 2>ReturnStmt@ (None)@]"
  | ReturnStmt(Some s) -> fprintf o "@[<hv 2>ReturnStmt@ (Some@ (%a))@]" print_expression s
