...
let externalizeFun fl =
  ...
  |> List.map (function
               |  FunctionDecl("main",dl,s) ->
                   FunctionDecl(renameExt "main",dl, iter_fun_stat (renameIt inFun) (fun _-> None) s)
            |  FunctionDecl(fn,dl,s) -> FunctionDecl(fn,dl,iter_fun_stat (renameIt inFun) (fun _-> None) s)
...
let _ =
  ...
  if !inline then List.iter (function
            | FunctionDecl(fn,al,s)-> funDecList:= StringMap.add fn (al,s) !funDecList;
            | VarDecl(_) ->()) defListExt;
  ...
  in
  if !doLinking then
    let o = if isLinux then Unix.open_process_out ("cc -no-pie -x assembler -o "^ !outputName ^" -")
    else Unix.open_process_out ("cc -no-pie -Wno-unused-command-line-argument -x assembler -o "^ !outputName ^" -") in
    output := o;
